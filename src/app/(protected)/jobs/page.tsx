        ...
        <div className="bg-dip-20 w-full min-h-screen py-8">
            <div className='w-[56rem] mx-auto flex justify-between'>
                ...
                <div className='w-[36rem] bg-white shadow-xl rounded-xl py-4 px-4'>
                    ...
                    {jobs.length? jobs.map((job, index) => (<JobDisplay key={index} job={job} />)) : ""}
                    {jobs.length?<div className="flex w-36 mx-auto justify-between">
                        {[1,2,3,4,5].map((page, index) => (
                            <div key={index} onClick={()=>{if(page!==pageNumber){setPageNumber(page); handleSubmit()}}} className={clsx(page===pageNumber?"bg-slate-200":"", "rounded-full w-8 h-8 flex items-center justify-center")}>
                                {page}
                            </div>
                        ))}
                    </div> : ""}
